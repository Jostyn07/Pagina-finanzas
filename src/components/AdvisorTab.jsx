...
SITUACIÓN FINANCIERA ACTUAL DEL USUARIO (${new Date().toLocaleDateString('es-CO', { month: 'long', year: 'numeric' })}):
...
1. Regla 50/30/20 de Elizabeth Warren (necesidades/deseos/ahorro)
2. 7 Baby Steps de Dave Ramsey (orden para construir libertad financiera)
3. The Psychology of Money de Morgan Housel (el comportamiento importa más que el conocimiento)
4. Rich Dad Poor Dad de Kiyosaki (adquirir activos, educación financiera)
5. The Richest Man in Babylon (págate primero, ahorra mínimo 10%)
6. I Will Teach You to Be Rich de Ramit Sethi (automatizar, sistemas de cuentas)
7. Behavioral Finance: ayudas al usuario a detectar sesgos cognitivos (aversión a pérdida, contabilidad mental, tunneling)

INSTRUCCIONES:
- Da consejos concretos, específicos y accionables basados en los datos reales del usuario
- Sé directo pero empático. No des sermones largos
- Usa los datos financieros para personalizar cada respuesta
- Menciona cifras específicas cuando sea relevante
- Si el usuario tiene deudas, aplica principios de Ramsey
- Detecta y señala sesgos cognitivos si ves patrones problemáticos
- Máximo 4-5 párrafos por respuesta, usa emojis con moderación
- Si algo es urgente (DTI alto, score bajo, gastos > ingresos), dilo claramente`
}
...
export default function AdvisorTab({ profile, monthlyIncome, expenses, totalDebt, healthScore, netWorth, savingsRate, debts, goals }) {
  ...
  async function sendMessage(text) {
    ...
    try {
      const systemPrompt = buildSystemPrompt(profile, monthlyIncome, expenses, debts || [], goals || [], healthScore, netWorth, savingsRate)
      ...
      const assistantText = data.content?.[0]?.text || 'Lo siento, no pude procesar tu pregunta. Intenta de nuevo.'
      ...
      await supabase.from('ai_conversations').insert({
        user_id: profile.id, role: 'assistant', content: assistantText,
        context: {}
      })
    } catch (err) {
      setMessages([...newMessages, {
        role: 'assistant',
        content: 'Tuve un problema conectándome. Verifica tu conexión e intenta de nuevo.'
      }])
    } finally {
      setLoading(false)
    }
